...
**الإصلاحات:**
- CRIT-01: إضافة `conn.close()` أو `return_connection(conn)` في `finally` block
- CRIT-03: استخدام `timedelta(hours=1)` بدل `hour + 1`
- CRIT-04: إضافة `__lt__` لكلاس `Event`
- CRIT-08: استخدام `timedelta(days=1)` بدل `day + 1`
- CRIT-09: استخدام `timedelta(days=7)` بدل `replace(day=...)`
- CRIT-10: تحويل لـ `int()` قبل `scaled()`
